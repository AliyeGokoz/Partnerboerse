...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "partnerboerse" because\n';
  ...
  $wnd.__gwt_activeModules['partnerboerse'] = {
    'moduleName' : 'partnerboerse',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/partnerboerse.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['partnerboerse'].superdevmode = true;

  var expectedSuffix = '/partnerboerse.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + partnerboerse + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:partnerboerse';
  var devModeSessionKey = '__gwtDevModeSession:partnerboerse';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/partnerboerse/partnerboerse.nocache.js';
  ...
  $wnd.__gwt_activeModules['partnerboerse'].canRedirect = true;
  $wnd.__gwt_activeModules['partnerboerse'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING partnerboerse';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/partnerboerse?user.agent=' + ua + '&_callback=' + callback;
